...
std::string Hashing::hashFromData(const std::vector<std::byte>& binaryData) {
...
std::optional<std::string> Hashing::hashFromFile(const FileOperations::FileInfo& fileInfo) {
    const auto bytes = FileOperations::readBinary(fileInfo.path);
